...
public class CardManager : MonoBehaviour
{
    ...
    [SerializeField] GameObject EarthHorn;
    [SerializeField] GameObject Entangle;
    [SerializeField] GameObject Explosion;
    [SerializeField] GameObject HurricaneOfFIRE;
    [SerializeField] GameObject Rain;
    [SerializeField] GameObject MagicMissile;

    public Transform HurriPos;

    GameObject cardObject;
    GameObject cardObject2;
    GameObject cardObject3;
    float r = 0;
    int i = 0;
    ...
    void CardSkill(Card card)
    {
        if(card.item2.name == "")
        {
            ...
            {
                case "시공간 폭발":
                    StartCoroutine(Skill_Explosion());
                    break;

                case "매직 미사일":
                    StartCoroutine(Skill_MaginMissile());
                    break;
                ...
                case "인탱글":
                    StartCoroutine(Skill_Entangle());
                    break;

                case "산성 비":
                    StartCoroutine(Skill_Rain());
                    break;

                case "대지의 뿔":
                    StartCoroutine(Skill_EarthHorn());
                    break;

                case "지진":
                    StartCoroutine(Skill_EarthQuake2());
                    break;
            }
    ...
    #region Skills
    IEnumerator Skill_EarthQuake2()
    {
        renderTexture = TestCamera.gameObject.GetComponent<RenderTextureDemo>().renderTexture;
        Camera.main.targetTexture = renderTexture;
        TestCamera.gameObject.GetComponent<RenderTextureDemo>().StartQuake();
        Handheld.Vibrate();
        TagBox = GameObject.FindGameObjectsWithTag("Enemy");
        for(i=0; i<TagBox.Length; i++)
        {
            TagBox[i].GetComponent<MoveManager>().TakeDamage(100);
        }
    ...
    {
        cardObject = Instantiate(HurricaneOfFIRE, HurriPos.position, Player.transform.rotation);
        //cardObject.transform.Translate(Vector2.right * moveX);
        // cardObject.transform.position = Vector3.MoveTowards(Player.transform.position, Player.transform.position * 10, 10);
        yield return null;
    }

    IEnumerator Skill_Explosion()
    {
        cardObject = Instantiate(Explosion, HurriPos.position, Player.transform.rotation);
        yield return null;
    }

    IEnumerator Skill_EarthHorn()
    {
        TagBox = GameObject.FindGameObjectsWithTag("Enemy");
        for(i=0; i<TagBox.Length; i++)
        {
            cardObject = Instantiate(EarthHorn, TagBox[i].gameObject.transform.position, Player.transform.rotation);
        }
       
        yield return null;
    }

    IEnumerator Skill_Entangle()
    {
        Enemy =  GameObject.FindGameObjectWithTag("Enemy");
        cardObject = Instantiate(Entangle, Enemy.transform.position, Player.transform.rotation);

        yield return null;

    }

    IEnumerator Skill_Rain()
    {
        cardObject = Instantiate(Rain, Camera.main.transform.position, Rain.transform.rotation);
        yield return new WaitForSeconds(1.2f);
        TagBox = GameObject.FindGameObjectsWithTag("Enemy");
        for (i = 0; i < TagBox.Length; i++)
        {
            TagBox[i].GetComponent<MoveManager>().TakeDamage(20);
        }
    ...
    IEnumerator Skill_MaginMissile()
    {
        cardObject = Instantiate(MagicMissile, Player.transform.position, Player.transform.rotation);
